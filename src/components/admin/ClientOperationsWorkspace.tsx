  ...
  return (
    <Card className="overflow-hidden border-slate-200 bg-white shadow-sm">
      <CardHeader className="gap-4 border-b border-slate-100 p-5">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-emerald-700">Gestão 360°</p><CardTitle className="mt-1 flex items-center gap-2 text-lg"><span className="grid h-9 w-9 place-items-center rounded-lg bg-slate-950"><UserRoundCog className="h-4 w-4 text-emerald-400" /></span>Carteira operacional <Badge variant="outline" className="ml-1 border-slate-200 bg-slate-50 text-slate-600">{visible.length} clientes</Badge></CardTitle><p className="mt-2 text-sm text-slate-500">Todos os clientes, priorizados por risco real e situação financeira.</p></div><Button type="button" variant="outline" onClick={exportPortfolio}><Download className="mr-2 h-4 w-4" />Exportar CSV</Button></div>
