  ...
  return (
    <section className="grid gap-5 xl:grid-cols-[0.8fr_1.2fr]">
      <Card className="border-slate-200 bg-white shadow-sm">
        <CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-lg bg-slate-950 text-emerald-400"><UserPlus className="h-5 w-5" /></div><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-emerald-700">Equipe de campo</p><CardTitle className="mt-1 text-lg">Novo representante</CardTitle><p className="text-sm text-slate-500">Crie um acesso individual para a equipe de campo.</p></div></div><a href="/representante" target="_blank" rel="noreferrer"><Button type="button" size="sm" variant="outline"><ExternalLink className="mr-1.5 h-3.5 w-3.5" />Abrir portal</Button></a></div></CardHeader>
        <CardContent>
          <form className="space-y-4" onSubmit={createRepresentative}>
            ...
            <Button className="w-full bg-emerald-600 font-semibold hover:bg-emerald-700" disabled={saving}>{saving ? 'Criando acesso…' : 'Criar representante'}</Button>
          </form>
        </CardContent>
      </Card>

      <Card className="border-slate-200 bg-white shadow-sm">
        <CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-emerald-700">Pipeline</p><CardTitle className="mt-1 flex items-center gap-2 text-lg"><Route className="h-5 w-5 text-emerald-600" />Funil de visitas</CardTitle><p className="mt-1 text-sm text-slate-500">Contatos captados em campo e autorizados para relacionamento.</p></div><Badge variant="outline" className="border-emerald-200 bg-emerald-50 text-emerald-700">{leads.filter((lead) => lead.marketing_consent).length} com marketing autorizado</Badge></div></CardHeader>
        <CardContent className="space-y-5">
          <div className="grid gap-3 sm:grid-cols-3"><div className="rounded-xl border border-slate-200 bg-slate-50 p-4"><Users className="h-4 w-4 text-slate-500" /><p className="mt-3 text-2xl font-semibold text-slate-950">{representatives.length}</p><p className="text-xs font-medium text-slate-500">Representantes</p></div><div className="rounded-xl border border-slate-200 bg-slate-50 p-4"><Route className="h-4 w-4 text-slate-500" /><p className="mt-3 text-2xl font-semibold text-slate-950">{leads.length}</p><p className="text-xs font-medium text-slate-500">Leads visitados</p></div><div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4"><BadgeCheck className="h-4 w-4 text-emerald-600" /><p className="mt-3 text-2xl font-semibold text-emerald-950">{leads.filter((lead) => lead.commercial_stage === 'won').length}</p><p className="text-xs font-medium text-emerald-700">Convertidos</p></div></div>
          <div className="space-y-3">{stageCounts.map((item) => <div key={item.stage}><div className="mb-1.5 flex justify-between text-sm font-semibold"><span>{stageLabels[item.stage] || item.stage}</span><span>{item.value}</span></div><div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-emerald-500" style={{ width: `${Math.max(8, (item.value / maxStage) * 100)}%` }} /></div></div>)}</div>
          ...
          <div className="grid gap-2 sm:grid-cols-2">{representatives.map((member) => <div key={member.id} className="rounded-xl border border-slate-200 bg-slate-50 p-3"><strong className="block text-sm">{member.display_name}</strong><span className="text-xs text-slate-500">{member.email}</span><p className="mt-2 text-xs font-semibold text-emerald-700"><MapPin className="mr-1 inline h-3 w-3" />{leadsByRepresentative.get(member.id) || 0} contato(s)</p></div>)}</div>
