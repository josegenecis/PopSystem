...
const CollapsibleSidebar = () => {
  ...
  return (
    <aside className={`
      fixed left-0 top-0 bottom-0 z-40 border-r border-[color:var(--app-primary-border)] bg-gradient-to-b from-[var(--app-primary-deep)] via-[var(--app-primary-deep)] to-[var(--app-primary-dark)] shadow-[8px_0_30px_-24px_rgba(var(--app-primary-rgb),0.55)] transition-all duration-300
      ${isMobile 
    ...
    >
      <nav className="flex h-full flex-col justify-between overflow-y-auto overscroll-contain touch-pan-y px-2 pb-20 pt-20 scrollbar-hide">
                ...
                <li key={link.to}>
                  <Link
                    ...
                    className={`flex items-center justify-center px-3 py-2 text-sm rounded-xl transition-colors ${
                      isActive
                        ? 'bg-[var(--app-primary)] text-white shadow-[0_12px_24px_-18px_rgba(var(--app-primary-rgb),0.75)]'
                        : `${(link as any).accent ? 'text-white' : 'text-white/90'} hover:bg-[var(--app-primary)] hover:text-white`
                    }`}
                  ...
                  <li key={link.to}>
                    <Link
                      ...
                      className={`flex items-center px-3 py-2 text-sm rounded-xl transition-colors ${
                        isActive
                          ? 'bg-[var(--app-primary)] font-medium text-white shadow-[0_12px_24px_-18px_rgba(var(--app-primary-rgb),0.75)]'
                          : 'text-white/90 hover:bg-[var(--app-primary)] hover:text-white'
                      }`}
                    >
              ...
              {visibleGroups.map((group) => {
                ...
                return (
                  <AccordionItem key={group.id} value={group.id} className="border-none">
                    <AccordionTrigger
                      className="rounded-xl px-3 py-2 text-sm text-white/90 hover:bg-[var(--app-primary)] hover:text-white hover:no-underline data-[state=open]:bg-[var(--app-primary)] data-[state=open]:text-white data-[state=open]:shadow-[0_12px_24px_-18px_rgba(var(--app-primary-rgb),0.8)]"
                    >
                    ...
                    <AccordionContent className="pb-0" style={{ animationDuration: '0.35s' }}>
                      <div className="mt-1 space-y-1 rounded-xl border border-white/15 bg-white/10 p-2 backdrop-blur-sm">
                          ...
                          return (
                            <Link
                              ...
                              className={`flex items-center px-3 py-2 text-sm rounded-lg transition-colors ${
                                isActive
                                  ? 'bg-[var(--app-primary)] font-medium text-white shadow-[0_10px_20px_-18px_rgba(var(--app-primary-rgb),0.7)]'
                                  : 'text-white/90 hover:bg-[var(--app-primary)] hover:text-white'
                              }`}
                            >
                  ...
                  <li key={link.to}>
                    <Link
                      ...
                      className={`flex items-center px-3 py-2 text-sm rounded-xl transition-colors ${
                        isActive
                          ? 'bg-[var(--app-primary)] font-medium text-white shadow-[0_12px_24px_-18px_rgba(var(--app-primary-rgb),0.75)]'
                          : `${link.accent ? 'text-white' : 'text-white/90'} hover:bg-[var(--app-primary)] hover:text-white`
                      }`}
                    >
                      <Icon size={18} className="mr-3 flex-shrink-0" />
                      <div className="min-w-0">
                        ...
                        {link.detail && (
                          <span className={`block truncate text-[10px] font-semibold ${isActive ? 'text-white/85' : 'text-white/65'}`}>
                            {link.detail}
                          </span>
                        )}
                      </div>
        ...
        <div className={`mt-auto border-t border-white/10 pt-4 pb-4 ${isOpen ? 'px-2' : 'px-0'}`}>
          {isOpen ? (
            <div className="space-y-2">
              <div className="flex items-center gap-3 rounded-xl border border-white/25 bg-white/10 px-3 py-2 shadow-[0_0_0_1px_rgba(255,255,255,0.06),0_0_24px_-14px_rgba(var(--app-primary-rgb),0.8)]">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="truncate text-xs text-white/65" title={user?.email}>
                    {user?.email}
                  </p>
                  {trialDaysLeft !== null && (
                  ...
                  {ifoodStatus && canAccessFeature('ifood') && (
                    <span className="mt-0.5 flex items-center gap-1.5 text-[10px] leading-tight text-white/70">
              ...
              <Button
                variant="ghost"
                className="w-full justify-start text-white hover:bg-[var(--app-primary)] hover:text-white"
              ...
              <Button 
                variant="ghost" 
                className="w-full justify-start text-white/90 hover:bg-[var(--app-primary)] hover:text-white"
            ...
            <div className="flex flex-col items-center gap-2">
              <Button
                variant="ghost"
                size="sm"
                className="h-10 w-full rounded-none p-0 text-white hover:bg-[var(--app-primary)] hover:text-white"
                onClick={() => setPinned(!isPinned)}
              ...
              <Button 
                variant="ghost" 
                size="sm" 
                className="h-10 w-full rounded-none p-0 text-white/75 hover:bg-[var(--app-primary)] hover:text-white"
                onClick={() => {
                  if (isMobile) {
                    closeSidebar();
                  }
