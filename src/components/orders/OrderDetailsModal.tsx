        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
          ...
          >
            <div className="grid min-w-0 grid-cols-1 gap-4 overflow-x-hidden p-4 sm:p-6 lg:grid-cols-2">
              ...
              {disputeOpen && (
                <div className="min-w-0 space-y-3 rounded-2xl border border-red-200 bg-red-50 p-4 shadow-sm sm:p-5 lg:col-span-2">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <h3 className="text-sm font-bold uppercase tracking-wide text-red-900">Negociação pendente no iFood</h3>
                    {negotiation?.expiresAt && (
                      <Badge variant="outline" className="border-red-300 bg-white text-red-800">
                        Responder até {formatDateTime(String(negotiation.expiresAt))}
                      </Badge>
                    )}
                  </div>
                  <p className="break-words text-sm text-red-950">
                    {String(negotiation?.message || 'O cliente abriu uma solicitação que precisa de resposta.')}
                  </p>
                  <div className="text-xs text-red-800">
                    {[negotiation?.action, negotiation?.handshakeType].filter(Boolean).join(' · ')}
                  </div>
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                    <Button disabled={respondingDispute} onClick={() => handleDisputeResponse('accept')} className="bg-emerald-600 hover:bg-emerald-700">
                      Aceitar
                    </Button>
                    <Button disabled={respondingDispute} onClick={() => handleDisputeResponse('reject')} variant="destructive">
                      Rejeitar
                    </Button>
                    <Button
                      disabled={respondingDispute || !Array.isArray(negotiation?.alternatives) || negotiation.alternatives.length === 0}
                      onClick={() => handleDisputeResponse('alternative')}
                      variant="outline"
              ...
              <div className="min-w-0 space-y-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
                ...
                <div className="min-w-0 space-y-2 text-sm text-slate-700">
                  ...
                  {paymentBrand && paymentMethods.length === 0 && (
                    <div className="flex min-w-0 justify-between gap-4 text-xs text-slate-600">
                      <span>Bandeira:</span>
                      <span className="min-w-0 break-words text-right font-medium">{paymentBrand}</span>
                    </div>
                  )}
                  {paymentMethodDetail && paymentMethods.length === 0 && (
                    <div className="flex min-w-0 justify-between gap-4 text-xs text-slate-600">
                  ...
                  {paymentMethods.length > 0 && (
                    <div className="min-w-0 space-y-1.5 rounded-lg bg-slate-50 p-2.5 text-xs text-slate-600">
                      <span className="font-medium text-slate-800">Formas de pagamento do iFood:</span>
                      {paymentMethods.map((method: IfoodPaymentMethodSummary, index: number) => (
                        <div key={`${method?.method || 'pagamento'}-${index}`} className="flex min-w-0 justify-between gap-3">
                          <span className="min-w-0 break-words">
                            {[method?.method, method?.brand || method?.wallet, method?.type].filter(Boolean).join(' · ') || 'Pagamento'}
                            {toNumber(method?.change_for) > 0 ? ` · troco para ${formatCurrency(method.change_for)}` : ''}
                          </span>
                          <span className="shrink-0 font-medium">{formatCurrency(method?.value || 0)}</span>
                        </div>
                      ))}
                    </div>
                  )}
                    ...
                    <div className="min-w-0 space-y-1 pt-1 text-xs text-slate-600">
                      ...
                      {benefitsSummary.map((benefit: any, index: number) => (
                        <div key={`${benefit?.description || 'benefit'}-${index}`} className="flex min-w-0 justify-between gap-3">
                          <span className="min-w-0 break-words">
                            {String(benefit?.description || 'Desconto iFood')}
                            {benefit?.responsibility ? ` · Responsável: ${benefit.responsibility}` : ''}
                          </span>
          ...
          {onStatusChange && order && ['pending', 'preparing', 'ready'].includes(order.status) && (
            <div className="shrink-0 border-t border-slate-200 bg-white px-4 py-3 sm:px-6 sm:py-4">
              ...
              {order.status === 'ready' && (
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                  <Button
                    onClick={() => handleStatusUpdate(isIfoodMerchantDelivery ? 'in_delivery' : 'delivered')}
                    className="h-11 rounded-xl bg-blue-600 text-sm font-bold shadow-sm hover:bg-blue-700"
                  >
                    <Truck className="mr-2 h-4 w-4" />
                    {isIfoodMerchantDelivery ? 'Saiu para entrega' : 'Finalizar pedido'}
                  </Button>
                  <Button
                    onClick={() => handleStatusUpdate('cancelled')}
                    variant="destructive"
