...
export const bridgePrintReceipt = async (params: {
  ...
  try {
    // O Pop Connect já inicia o bridge com a impressora salva na interface
    // desktop. Não devemos sobrescrevê-la com a configuração padrão (vazia)
    // do PWA. A conexão explícita só é necessária quando o próprio PWA
    // informou um endereço.
    ...
    const printed = await sendAndWait(
      ws,
      'print_receipt',
      params.payload,
      'printed_receipt',
...
export const bridgeOpenCashDrawer = async (params: {
  ...
  const opened = await new Promise<boolean>((resolve) => {
    const timeout = window.setTimeout(() => resolve(false), Math.min(2500, timeoutMs))
    ws.onopen = () => {
      window.clearTimeout(timeout)
      resolve(true)
    }
  ...
  try {
    const resultPromise = waitForEvent(ws, 'cash_drawer_opened', timeoutMs)
    ws.send(JSON.stringify({ action: 'open_cash_drawer', payload: { connector: 0 } }))
