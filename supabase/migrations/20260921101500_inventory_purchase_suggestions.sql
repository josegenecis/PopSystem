-- Shared purchase suggestions for the stock screen and Pop Agent.
-- Combines configured minimums with recent real consumption.
...
begin
  ...
  with ingredient_consumption as (
    select
      ...
      coalesce(sum(case
        when movement.movement_type in ('sale', 'out', 'loss') then abs(movement.quantity)
        when movement.movement_type = 'return' then -abs(movement.quantity)
        else 0
      end), 0)::numeric as consumed
  ...
  ingredient_rows as (
    select
      ...
      ingredient.id as item_id,
      ingredient.name,
      coalesce(ingredient.current_stock, 0)::numeric as current_stock,
      coalesce(ingredient.min_stock, 0)::numeric as minimum_stock,
      ingredient.unit as stock_unit,
      coalesce(nullif(ingredient.purchase_unit, ''), ingredient.unit) as purchase_unit,
      greatest(coalesce(ingredient.purchase_conversion, 1), 0.000001)::numeric as purchase_conversion,
      greatest(coalesce(consumption.consumed, 0), 0)::numeric as consumed_in_period,
      (greatest(coalesce(consumption.consumed, 0), 0) / v_history_days)::numeric as average_daily_consumption
  ...
  product_consumption as (
    select
      movement.product_id,
      coalesce(sum(case
        when movement.type = 'sale' then abs(movement.quantity)
        when movement.type = 'return' then -abs(movement.quantity)
        else 0
      end), 0)::numeric as consumed
  ...
  product_rows as (
    select
      ...
      product.name,
      coalesce(product.stock_quantity, 0)::numeric as current_stock,
      coalesce(product.low_stock_threshold, 0)::numeric as minimum_stock,
      'un'::text as stock_unit,
      'un'::text as purchase_unit,
      1::numeric as purchase_conversion,
      greatest(coalesce(consumption.consumed, 0), 0)::numeric as consumed_in_period,
      (greatest(coalesce(consumption.consumed, 0), 0) / v_history_days)::numeric as average_daily_consumption
    from public.products product
    left join product_consumption consumption on consumption.product_id = product.id
    where product.user_id = p_store_user_id
  ...
  calculated as (
    select *,
      greatest(minimum_stock, average_daily_consumption * v_cover_days)::numeric as target_stock
    from candidates
  ),
  suggestions as (
    select *,
      greatest(target_stock - current_stock, 0)::numeric as suggested_stock_quantity,
      ceil(greatest(target_stock - current_stock, 0) / purchase_conversion)::numeric as suggested_purchase_quantity,
      case
        when current_stock <= 0 then 'Sem estoque'
        when current_stock <= minimum_stock then 'Abaixo do estoque mínimo'
        else 'Cobertura abaixo de ' || v_cover_days || ' dias'
      end as reason,
      case
        when average_daily_consumption > 0 then round(current_stock / average_daily_consumption, 1)
        else null
      end as days_remaining
    from calculated
    where target_stock > current_stock
      and (minimum_stock > 0 or average_daily_consumption > 0)
  )
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'minimum_stock', round(minimum_stock, 3),
    ...
    'cover_days', v_cover_days
  ) order by
    case when current_stock <= 0 then 0 when current_stock <= minimum_stock then 1 else 2 end,
...
comment on function public.get_inventory_purchase_suggestions(uuid, integer, integer) is
  'Sugere compras usando saldo, estoque mínimo e consumo real recente; compartilhada pela tela de estoque e Pop Agente.';
